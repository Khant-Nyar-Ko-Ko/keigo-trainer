...
import { WordGradeResult, gradeWordAnswer } from "@/lib/word-grade";
import {
  WordQuestion,
  loadWordProgress,
  pickWord,
...
export default function WordQuiz() {
  ...
  const [result, setResult] = useState<WordGradeResult | null>(null);
  const [stats, setStats] = useState({ correct: 0, total: 0 });
  const [sessionComplete, setSessionComplete] = useState(false);

  useEffect(() => {
    setQuestion(pickWord(loadWordProgress()));
  }, []);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const graded = gradeWordAnswer(input, question.word, question.target);
    setSubmittedAnswer(input);
    setResult(graded);
    setStats((s) => ({ correct: s.correct + (graded.correct ? 1 : 0), total: s.total + 1 }));
    recordAttempt("words", graded.correct);

    if (!graded.correct) {
      recordWordMiss(question.word.id, question.target);
    }
  }
  ...
  function handleNext() {
    ...
    setQuestion(pickWord(loadWordProgress(), question ?? undefined));
    resetQuestionState();
  }
      ...
      {sessionComplete ? (
        ...
        <>
          ...
          >
            ...
            <div lang="ja" className="mt-1 font-display text-5xl text-ink">
              {question.word.plain}
            </div>
            <div className="text-ink-faint">
              <span lang="ja">{question.word.reading}</span> — {question.word.meaning}
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            <input
              ...
              disabled={!!result}
              placeholder="Type the honorific form..."
              autoFocus
